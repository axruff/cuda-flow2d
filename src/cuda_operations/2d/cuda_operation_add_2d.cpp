...
#include "cuda_operation_add_2d.h"
...
CudaOperationAdd2D::CudaOperationAdd2D()
  : CudaOperationBase("CUDA Add 2D")
{
}

bool CudaOperationAdd2D::Initialize(const OperationParameters* params)
{
  initialized_ = false;
  ...
  std::strcat(exec_path, "/kernels/add_2d.ptx");

  if (!CheckCudaError(cuModuleLoad(&cu_module_, exec_path))) {
    if (!CheckCudaError(cuModuleGetFunction(&cuf_add_, cu_module_, "add_2d"))) {
      size_t const_size;
...
void CudaOperationAdd2D::Execute(OperationParameters& params)
{
  if (!IsInitialized()) {
    return;
  }
